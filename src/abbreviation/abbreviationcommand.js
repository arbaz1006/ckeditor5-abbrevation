...
export default class AbbreviationCommand extends Command {
  ...
  execute({ abbr, title }, id) {
      ...
      if (selection.isCollapsed) {
        ...
        if (this.value) {
          const { end: positionAfter } = model.insertContent(
            writer.createText(abbr, { abbreviation: { title: title, id: id } }),
            this.value.range
          );
        ...
        // If the abbreviation is empty, do not do anything.
          ...
          attributes.set("abbreviation", { title: title, id: id });
        ...
        for (const range of ranges) {
          writer.setAttribute("abbreviation", { title: title, id: id }, range);
        }
      }
    });
    ...
    if (abbr !== "" && title !== "") {
      ...
      syncData("customtag", "update", abbr, title, id);
    }
  }
}
