...
export default class AbbreviationUI extends Plugin {
  ...
  init() {
    ...
    editor.ui.componentFactory.add("abbreviation", () => {
      ...
      button.set({
        icon: '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100"><circle cx="50" cy="50" r="40" fill="none" stroke="black" stroke-width="2" /><text x="45" y="58" font-family="Arial" font-size="40" fill="black">i</text></svg>',
        withText: true,
        tooltip: "Please select text to add custom tag",
        tooltipPosition: "s",
        label: "Custom Tag",
      });
  ...
  _createFormView() {
    ...
    this.listenTo(formView, "submit", () => {
      ...
      //add tag only if there is title/description available
      if (value.abbr !== "" && value.title !== "") {
        //generate id only for new tags, use already available id if updating the tag
        if (selection.hasAttribute("abbreviation")) {
          editor.execute("addAbbreviation", value, attributeValue.id);
        } else {
          const { getTagId } = this.editor.config.get("customCalls");
          getTagId((transTagId) => {
            editor.execute("addAbbreviation", value, transTagId);
          });
        }
      }
    ...
    // Hide the form view after clicking the "Remove" button.
    this.listenTo(formView, "delete", () => {
      editor.execute("removeAbbreviation");
      this._hideUI();
    });
  ...
  _showUI() {
    ...
    //Disable erase button when adding abbreviation for the first time
    if (!selection.hasAttribute("abbreviation")) {
      this.formView.removeTag.isEnabled = false;
    } else {
      this.formView.removeTag.isEnabled = true;
    }
